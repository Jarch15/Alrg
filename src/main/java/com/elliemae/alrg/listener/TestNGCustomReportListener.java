package com.elliemae.alrg.listener;
...
public class TestNGCustomReportListener implements IReporter{
	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites,
		...
		startHtml(writer);
		writeReportTitle(ALRGConsts.REPORT_FILE_TITLE);
	...
	protected PrintWriter createWriter(String outdir) throws IOException {
		...
		return new PrintWriter(new BufferedWriter(new FileWriter(new File(outdir, ALRGConsts.REPORT_FILE_NAME))));
	}
	...
	private void resultSummary(ISuite suite, IResultMap tests, String testname,
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				DateFormat formatter = new SimpleDateFormat("HH:mm:ss aaa");
	...
	public void generateSuiteSummaryReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r : tests.values()) {
				...
				summaryCell(suite.getParameter("browserName"), true);
				writer.println("</td>");
							
				SimpleDateFormat summaryFormat = new SimpleDateFormat("HH:mm:ss aaa");
